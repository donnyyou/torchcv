...
CONFIG_FILE='configs/seg/ade20k/fs_pspnet_ade20k_seg.conf'
MAX_ITERS=150000
...
if [[ "$1"x == "train"x ]]; then
  ${PYTHON} -u main.py --hypes ${CONFIG_FILE} --drop_last y --phase train --gathered n --loss_balance y \
  ...
  cd metrics/seg/
  ${PYTHON} -u ade20k_evaluator.py --hypes "../../"${CONFIG_FILE} \
