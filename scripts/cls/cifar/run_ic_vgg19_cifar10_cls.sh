...
if [[ "$1"x == "train"x ]]; then
  ${PYTHON} -u main.py --hypes ${HYPES_FILE} --phase train --gathered n --loss_balance y --model_name ${MODEL_NAME} \
  ...
  ${PYTHON} -u main.py --hypes ${HYPES_FILE} --phase train --loss_balance y \
  ...
  ${PYTHON} -u main.py --hypes ${HYPES_FILE} --phase test --gpu 0 1 2 3 --gathered n \
