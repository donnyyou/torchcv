...
if [[ "$1"x == "train"x ]]; then
  ${PYTHON} -u main.py --hypes ${CONFIG_FILE} --phase train --gpu 1 --model_name ${MODEL_NAME} --tag ${TAG} \
                       --data_dir ${DATA_DIR} --max_epoch ${MAX_EPOCH} \
                       --checkpoints_name ${CHECKPOINTS_NAME}  2>&1 | tee ${LOG_FILE}

elif [[ "$1"x == "resume"x ]]; then
  ${PYTHON} -u main.py --hypes ${CONFIG_FILE} --phase train --gpu 0 --model_name ${MODEL_NAME} --tag ${TAG} \
  ...
  ${PYTHON} -u main.py --hypes ${CONFIG_FILE} --phase test --gpu 0 --tag ${TAG} \
