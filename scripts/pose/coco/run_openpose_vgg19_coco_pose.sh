...
if [[ "$1"x == "train"x ]]; then
  ${PYTHON} -u main.py --hypes ${CONFIG_FILE} --phase train --gpu 0 1 2 3 \
  ...
  ${PYTHON} -u main.py --hypes ${CONFIG_FILE} --phase train --log_to_file n --gpu 0 1 2 3 \
                       --data_dir ${DATA_DIR} --loss_type ${LOSS_TYPE} --model_name ${MODEL_NAME} \
  ...
  ${PYTHON} -u main.py --hypes ${CONFIG_FILE} --phase test --model_name ${MODEL_NAME} \
  ...
  cd metrics/det/
  ${PYTHON} -u voc_evaluator.py --hypes "../../../"${CONFIG_FILE} \
